...
# Add HuggingFace model configuration
HF_CONFIGS = {
    # "Qwen/Qwen3-1.7B": {
...
# ======================== HuggingFace Model Features ================

class HFModelManager:
    """Simplified HuggingFace model manager"""
    def __init__(self, model_name):
        self.model_name = model_name
        self.config = HF_CONFIGS.get(model_name, {})
        self.model = None
        self.tokenizer = None
    ...
    def initialize(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(self.config.get("model_path", self.model_name))
        self.model = AutoModelForCausalLM.from_pretrained(
            self.config.get("model_path", self.model_name),
...
# Model manager dictionary
hf_model_managers = {}

def get_hf_model_manager(model_name):
    """Get or create a model manager"""
    if (model_name not in hf_model_managers):
        hf_model_managers[model_name] = HFModelManager(model_name)
        hf_model_managers[model_name].initialize()
    return hf_model_managers[model_name]
...
def call_model_api_with_retry(model_name, facts=None, hypothesis=None, prompt_style=None, max_retries=3, input_data=None):
    ...
    if model_name in HF_CONFIGS:
        return call_hf_model_with_retry(model_name, facts, hypothesis, prompt_style, max_retries, input_data)
...
def call_hf_model_with_retry(model_name, facts=None, hypothesis=None, prompt_style=None, max_retries=3, input_data=None):
    """Generate answers using HuggingFace models"""
    style = PROMPT_STYLES[prompt_style] if prompt_style else PROMPT_STYLES["direct"]
    suffix = style["suffix"]
    if prompt_style == "fewshot":
        suffix = suffix.format(fewshot_examples=FEWSHOT_EXAMPLES)
    ...
    system_message = style["system"]
    
    # Build complete prompt
    prompt = f"<s>[INST] {system_message}\n\n{user_message} [/INST]"
    
    model_manager = get_hf_model_manager(model_name)
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Exception for HF model {model_name}: {e}. Retrying...")
            time.sleep(1)
...
def process_problems_for_model(data, model_name, max_workers=100):
    # Special handling for local HuggingFace models
    if model_name in HF_CONFIGS:
        return process_problems_for_hf_model(data, model_name)
        
    # Use original concurrent processing for API models
    results = []
...
def process_problems_for_hf_model(data, model_name):
    """
    Processing function specifically designed for local HuggingFace models
    Simplified version: directly process all problems sequentially
    """
    print(f"Using local model processing method for {model_name}...")
    results = []
    total = len(data)
    
    # Prepare model manager beforehand
    model_manager = get_hf_model_manager(model_name)
    
    # Process all problems sequentially
    for i, problem in enumerate(tqdm(data, desc=f'Processing items for {model_name}')):
        try:
            result = process_problem(problem, model_name)
            results.append(result)
        except Exception as exc:
            print(f"Problem {i} generated an exception: {exc}")
            continue
...
def main():
    ...
    all_models = list(API_CONFIGS.keys()) + list(HF_CONFIGS.keys())
    ...
    try:
        for model_name in all_models:
            ...
            # Use the modified processing function that automatically determines model type
        ...
        for manager in hf_model_managers.values():
            manager.cleanup()
